# Enum: TrophicLevelEnum 




_Enumeration of trophic levels for organisms._



URI: [basalt_schema:TrophicLevelEnum](https://w3id.org/MONet/basalt-schema/TrophicLevelEnum)

## Permissible Values
| Value | Meaning | Description |
| --- | --- | --- |
| autotroph | None | Organism that produces complex organic compounds from simple substances |
| carboxydotroph | None | Organism that oxidizes carbon monoxide as energy source |
| chemoautolithotroph | None | Chemolithoautotroph using inorganic compounds |
| chemoautotroph | None | Organism using chemical energy to synthesize organic compounds |
| chemoheterotroph | None | Organism obtaining energy by oxidizing organic compounds |
| chemolithoautotroph | None | Organism using inorganic electron donors |
| chemolithotroph | None | Organism deriving energy from inorganic compounds |
| chemoorganoheterotroph | None | Organism using organic compounds as electron donors |
| chemoorganotroph | None | Organism obtaining energy from organic compounds |
| chemosynthetic | None | Organism using chemosynthesis |
| chemotroph | None | Organism obtaining energy from chemical reactions |
| copiotroph | None | Organism thriving in nutrient-rich environments |
| diazotroph | None | Organism capable of nitrogen fixation |
| facultative | None | Organism capable of multiple metabolic modes |
| heterotroph | None | Organism requiring organic compounds for nutrition |
| lithoautotroph | None | Organism using inorganic substrates |
| lithoheterotroph | None | Organism using inorganic electron donors with organic carbon |
| lithotroph | None | Organism using inorganic electron donors |
| methanotroph | None | Organism using methane as carbon/energy source |
| methylotroph | None | Organism using reduced one-carbon compounds |
| mixotroph | None | Organism combining autotrophy and heterotrophy |
| obligate | None | Organism restricted to one metabolic mode |
| oligotroph | None | Organism thriving in nutrient-poor environments |
| organoheterotroph | None | Organism using organic compounds |
| organotroph | None | Organism obtaining electrons from organic compounds |
| osmotroph | None | Organism absorbing dissolved nutrients |
| photoheterotroph | None | Organism using light with organic carbon |
| photoautotroph | None | Organism using light to fix carbon dioxide |
| photolithoautotroph | None | Photosynthetic organism using inorganic electron donors |
| photolithotroph | None | Organism using light and inorganic electron donors |
| phototroph | None | Organism using light as energy source |




## Slots

| Name | Description |
| ---  | --- |
| [trophic_level](trophic_level.md) | Trophic levels are the feeding position in a food chain |










## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/MONet/basalt-schema






## LinkML Source

<details>
```yaml
name: TrophicLevelEnum
description: Enumeration of trophic levels for organisms.
from_schema: https://w3id.org/MONet/basalt-schema
rank: 1000
permissible_values:
  autotroph:
    text: autotroph
    description: Organism that produces complex organic compounds from simple substances
  carboxydotroph:
    text: carboxydotroph
    description: Organism that oxidizes carbon monoxide as energy source
  chemoautolithotroph:
    text: chemoautolithotroph
    description: Chemolithoautotroph using inorganic compounds
  chemoautotroph:
    text: chemoautotroph
    description: Organism using chemical energy to synthesize organic compounds
  chemoheterotroph:
    text: chemoheterotroph
    description: Organism obtaining energy by oxidizing organic compounds
  chemolithoautotroph:
    text: chemolithoautotroph
    description: Organism using inorganic electron donors
  chemolithotroph:
    text: chemolithotroph
    description: Organism deriving energy from inorganic compounds
  chemoorganoheterotroph:
    text: chemoorganoheterotroph
    description: Organism using organic compounds as electron donors
  chemoorganotroph:
    text: chemoorganotroph
    description: Organism obtaining energy from organic compounds
  chemosynthetic:
    text: chemosynthetic
    description: Organism using chemosynthesis
  chemotroph:
    text: chemotroph
    description: Organism obtaining energy from chemical reactions
  copiotroph:
    text: copiotroph
    description: Organism thriving in nutrient-rich environments
  diazotroph:
    text: diazotroph
    description: Organism capable of nitrogen fixation
  facultative:
    text: facultative
    description: Organism capable of multiple metabolic modes
  heterotroph:
    text: heterotroph
    description: Organism requiring organic compounds for nutrition
  lithoautotroph:
    text: lithoautotroph
    description: Organism using inorganic substrates
  lithoheterotroph:
    text: lithoheterotroph
    description: Organism using inorganic electron donors with organic carbon
  lithotroph:
    text: lithotroph
    description: Organism using inorganic electron donors
  methanotroph:
    text: methanotroph
    description: Organism using methane as carbon/energy source
  methylotroph:
    text: methylotroph
    description: Organism using reduced one-carbon compounds
  mixotroph:
    text: mixotroph
    description: Organism combining autotrophy and heterotrophy
  obligate:
    text: obligate
    description: Organism restricted to one metabolic mode
  oligotroph:
    text: oligotroph
    description: Organism thriving in nutrient-poor environments
  organoheterotroph:
    text: organoheterotroph
    description: Organism using organic compounds
  organotroph:
    text: organotroph
    description: Organism obtaining electrons from organic compounds
  osmotroph:
    text: osmotroph
    description: Organism absorbing dissolved nutrients
  photoheterotroph:
    text: photoheterotroph
    description: Organism using light with organic carbon
  photoautotroph:
    text: photoautotroph
    description: Organism using light to fix carbon dioxide
  photolithoautotroph:
    text: photolithoautotroph
    description: Photosynthetic organism using inorganic electron donors
  photolithotroph:
    text: photolithotroph
    description: Organism using light and inorganic electron donors
  phototroph:
    text: phototroph
    description: Organism using light as energy source

```
</details>
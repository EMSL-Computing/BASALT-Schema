# Enum: SyntheticEnvironmentEnum 



URI: [basalt_schema:SyntheticEnvironmentEnum](https://emsl-computing.github.io/BASALT-Schema/elements/SyntheticEnvironmentEnum)

## Permissible Values
| Value | Meaning | Description | Additional Info |
| --- | --- | --- | --- |
| pore_scale_micromodels | None |  | Title: Pore-scale micromodels<br>|
| rhizochip | None |  | Title: RhizoChip<br>|
| subtap | None |  | Title: SubTap microbial and rhizosphere platforms<br>|
| three_d_bioprinted_synthetic_soil_aggregates | None |  | Title: 3-D Bioprinted Synthetic Soil Aggregates<br>|
| pore2chip | None |  | Title: Pore2Chip<br>|




## Slots

| Name | Description |
| ---  | --- |
| [synth_env_design](synth_env_design.md) | The design of the synthetic environment that was created for experimentation |










## Identifier and Mapping Information





### Schema Source


* from schema: https://emsl-computing.github.io/BASALT-Schema






## LinkML Source

<details>
```yaml
name: SyntheticEnvironmentEnum
from_schema: https://emsl-computing.github.io/BASALT-Schema
rank: 1000
permissible_values:
  pore_scale_micromodels:
    text: pore_scale_micromodels
    title: Pore-scale micromodels
  rhizochip:
    text: rhizochip
    title: RhizoChip
  subtap:
    text: subtap
    title: SubTap microbial and rhizosphere platforms
  three_d_bioprinted_synthetic_soil_aggregates:
    text: three_d_bioprinted_synthetic_soil_aggregates
    title: 3-D Bioprinted Synthetic Soil Aggregates
  pore2chip:
    text: pore2chip
    title: Pore2Chip

```
</details>
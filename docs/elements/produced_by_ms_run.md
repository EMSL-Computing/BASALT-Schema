

# Slot: produced_by_ms_run 


_a reference to the data generation activity that produced instrument data_





URI: [basalt_schema:produced_by_ms_run](https://emsl-computing.github.io/BASALT-Schema/elements/produced_by_ms_run)
Alias: produced_by_ms_run

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [MassSpectrometryInstrumentData](MassSpectrometryInstrumentData.md) | Raw data files output from a mass spectrometry instrument |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [MassSpectrometryDataGenerationActivity](MassSpectrometryDataGenerationActivity.md) |
| Domain Of | [MassSpectrometryInstrumentData](MassSpectrometryInstrumentData.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://emsl-computing.github.io/BASALT-Schema




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | basalt_schema:produced_by_ms_run |
| native | basalt_schema:produced_by_ms_run |




## LinkML Source

<details>
```yaml
name: produced_by_ms_run
description: a reference to the data generation activity that produced instrument
  data
from_schema: https://emsl-computing.github.io/BASALT-Schema
rank: 1000
alias: produced_by_ms_run
domain_of:
- MassSpectrometryInstrumentData
range: MassSpectrometryDataGenerationActivity

```
</details>


# Slot: rooting medium solidifier (root_med_solid) 


_Specification of the solidifying agent in the culture rooting medium._





URI: [basalt_schema:root_med_solid](https://emsl-computing.github.io/BASALT-Schema/elements/root_med_solid)
Alias: root_med_solid

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [PlantSample](PlantSample.md) | A sample containing plant material |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [String](String.md) |
| Domain Of | [PlantSample](PlantSample.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://emsl-computing.github.io/BASALT-Schema




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | basalt_schema:root_med_solid |
| native | basalt_schema:root_med_solid |




## LinkML Source

<details>
```yaml
name: root_med_solid
description: Specification of the solidifying agent in the culture rooting medium.
title: rooting medium solidifier
from_schema: https://emsl-computing.github.io/BASALT-Schema
rank: 1000
alias: root_med_solid
domain_of:
- PlantSample
range: string

```
</details>
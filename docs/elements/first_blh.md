

# Slot: first boundary layer height (first_blh) 


_First boundary layer height candidate (meters) (Unit: m)_





URI: [basalt_schema:first_blh](https://emsl-computing.github.io/BASALT-Schema/elements/first_blh)
Alias: first_blh

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [AerosolArmSample](AerosolArmSample.md) | An aerosol sample collected by the ARM facility |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [Float](Float.md) |
| Domain Of | [AerosolArmSample](AerosolArmSample.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |










## Identifier and Mapping Information





### Schema Source


* from schema: https://emsl-computing.github.io/BASALT-Schema




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | basalt_schema:first_blh |
| native | basalt_schema:first_blh |




## LinkML Source

<details>
```yaml
name: first_blh
description: 'First boundary layer height candidate (meters) (Unit: m)'
title: first boundary layer height
from_schema: https://emsl-computing.github.io/BASALT-Schema
rank: 1000
alias: first_blh
domain_of:
- AerosolArmSample
range: float

```
</details>
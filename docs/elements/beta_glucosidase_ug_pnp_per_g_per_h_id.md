

# Slot: beta_glucosidase_ug_pnp_per_g_per_h_id 



URI: [basalt_schema:beta_glucosidase_ug_pnp_per_g_per_h_id](https://w3id.org/MONet/basalt-schema/beta_glucosidase_ug_pnp_per_g_per_h_id)
Alias: beta_glucosidase_ug_pnp_per_g_per_h_id

<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [EnzymeProduct](EnzymeProduct.md) | Enzyme activity analysis product, typically derived via colorimetric assay of... |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [QuantityValue](QuantityValue.md) |
| Domain Of | [EnzymeProduct](EnzymeProduct.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |
### Slot Characteristics

| Property | Value |
| --- | --- |
| Owner | [EnzymeProduct](EnzymeProduct.md) |












## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/MONet/basalt-schema




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | basalt_schema:beta_glucosidase_ug_pnp_per_g_per_h_id |
| native | basalt_schema:beta_glucosidase_ug_pnp_per_g_per_h_id |




## LinkML Source

<details>
```yaml
name: beta_glucosidase_ug_pnp_per_g_per_h_id
from_schema: https://w3id.org/MONet/basalt-schema
rank: 1000
alias: beta_glucosidase_ug_pnp_per_g_per_h_id
owner: EnzymeProduct
domain_of:
- EnzymeProduct
range: QuantityValue

```
</details>
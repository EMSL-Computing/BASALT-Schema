...
def _get_pyrange(t: TypeDefinition, sv: SchemaView) -> str:
    if t is None:
        return "str"  # Default fallback for None types
...
DEFAULT_IMPORTS = (
    ...
    + Import(module="re")
    + Import(module="sys")
    + Import(module="uuid", objects=[ObjectImport(name="UUID")])
    + Import(
        module="typing",
        objects=[
    ...
    * :meth:`~.LifecycleMixin.after_render_template`
    ...
    def generate_python_range(self, slot_range, slot_def: SlotDefinition, class_def: ClassDefinition) -> str:
        ...
        if slot_def.designates_type:
            ...
            pyrange = "str"
        elif slot_range.lower() in ['uuid']:
            pyrange = "UUID"
        elif slot_range.lower() in ['numeric']:
            pyrange = "Decimal"
        elif slot_range == 'timestamp_tz':
            pyrange = "datetime"
        else:
